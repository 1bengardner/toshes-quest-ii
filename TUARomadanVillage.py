...
class RomadanVillage:

    name = "Romadan Village"
    ...
    def entrance(self, selectionIndex=None):
        ...
        if selectionIndex == 0:
            X = 1
            Y = 1
            return self.actions({'area': "Romadan Hideout",
                                 'coordinates': (X, Y)})
    ...
    def corner2(self, selectionIndex=None):
        ...
        if "Bayezin" not in self.c.flags:
            self.text = ("You stand in front of a complex. One dwelling" +
                         " has a sign which reads \"Romada Inn.\"")
        self.menu = ["Enter the Romada Inn."]
        return self.actions()
    ...
    def romadaInn(self, selectionIndex=None):
        self.view = "travel"
        self.imageIndex = 11
        self.text = None
        self.helpText = None
...
["Ok, if you wanna know something, don't forget to drink water." +
 ...
 "Stay away from the central plaza. All of the Romadans are out for blood.",
 ...
 " security's gotten really tight lately."
            ...
            if (self.c.euros >= price or
                'Romadan Village Room Level' in self.c.flags):
                X = 3
                Y = 9
                return self.actions({'area': "Romadan Village",
                                     'coordinates': (X, Y)})
            ...
            if "Rested" in self.c.flags:
                ...
                del self.c.flags['Romadan Village Room Level']
                  ...
                  self.c.flags['Romadan Village Room Level'] < self.c.level):
        ...
        if "Romadan Village Room Level" in self.c.flags:
    ...
    def bedroom(self, selectionIndex=None):
        ...
        if selectionIndex == 0:
            ...
            return self.actions({'area': "Romadan Village",
                                 'coordinates': (X, Y)})
        if "Romadan Village Room Level" not in self.c.flags:
            self.c.euros -= price
